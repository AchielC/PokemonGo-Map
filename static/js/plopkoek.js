...
var announceQueue = []
var playingAnnounceQueue = false;
...
function initSidebar () {
  ...
  $('#announce-switch').prop('checked', Store.get('announceSound'))
...
function playAnnounceQueue() {
  if(playingAnnounceQueue) {
    return;
  }
  playingAnnounceQueue = true;
  _playAnnounceQueue();
}

function _playAnnounceQueue() {
...
function setupPokemonMarker (item, skipNotification, isBounceDisabled) {
  ...
  if (notifiedPokemon.indexOf(item['pokemon_id']) > -1 || notifiedRarity.indexOf(item['pokemon_rarity']) > -1) {
    if (!skipNotification) {
      ...
      if (Store.get('announceSound')) {
        announceQueue.push(item['pokemon_name'])
      }
